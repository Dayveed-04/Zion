  ...
  return (
    <div className="min-h-screen bg-white">
      <section className="max-w-4xl mx-auto px-4 py-12 text-center mb-16">
        ...
        <p className="text-sm md:text-basetext-gray-600">
      ...
      <section className="max-w-4xl mx-auto px-4 mb-12">
        <div className="flex flex-col items-center space-y-12">
          <div className="flex flex-col items-center">
            <div className="w-32 h-32 sm:w-48 sm:h-48 rounded-full overflow-hidden border-4 border-[#DB241E] mb-4">
              <img
            ...
            <p className="text-xs sm:text-sm text-gray-600 mb-1">{apostle.title}</p>
            <h3 className="text-base sm:text-lg font-bold text-gray-900">{apostle.name}</h3>
          </div>

          <div className="grid grid-cols-2 gap-16">
            {pastors.map((pastor) => (
              <div key={pastor.id} className="flex flex-col items-center">
                <div className="w-24 h-24 sm:w-32 sm:h-32 rounded-full overflow-hidden border-4 border-[#DB241E] mb-4">
                  <img
                    src={pastor.imageUrl}
                    alt={pastor.name}
                    className="w-full h-full object-cover"
      ...
      <section className="flex justify-center pb-32">
          ...
          <Button text="VIEW LIST OF MINISTERS" bgColor="bg-[#DB241E]" onClick={() => navigate('/ministers/all')}/>
      </section>
    </div>
  );
};
